package controller;

public class Administrateur extends Utilisateur {

  /**
   * Constructeur de la classe particulier.
   * @param p prenom du particulier
   * @param m mot de passe du particulier
   * @param pre prenom du particulier
   * @param n nom du particulier
   * @param ad adresse du particulier
   * @param ma mail du particulier
   */

  Administrateur(String p, String m, String pre, String n, String ad, String ma) {
    super(p, m, pre, n, ad, ma);
  }
}
