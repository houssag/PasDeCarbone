...
public abstract class Utilisateur {
	...
	Utilisateur(String p, String m, String pre, String n, String ad, String ma) {
		...
		// On intialise le pseudo le mail et le mot de passe si ils ne sont pas vides
		if (p != null && m != null && ma != null && p.isEmpty() != true && m.isEmpty() != true
			...
			if (!estInvalide(pre) && !estInvalide(n)) {
				...
				// On intialise l'adresse si elle n'est pas vide (elle peut contenir des
				// chiffres)
				if (ad.isEmpty() != true) {
					this.adresse = ad;
				}
			} else {
				throw new IllegalArgumentException("L'un des arguments contient un charactère non autorisé");
			}
		} else {
	...
	/**
	 * Permet de changer le prénom d'un utilisateur si il ne contient pas de
	 * chiffre.
	 * 
	...
	/**
	 * Permet de savoir si une chaîne passée en paramètre ne contient que des
	 * caractères contenus dans un prénom.
	 * 
	 * @param s La chaîne de caractère que l'on veut analyser
	 * @return un booléen, vrai si la chaîne ne correspond pas aux exigences, faux
	 *         sinon
	 */
